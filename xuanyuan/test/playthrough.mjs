...
async function fight() {
    ...
    for (let turn = 0; turn < 150; turn++) {
        ...
        // Heal before swinging if this hero is badly hurt — a bot that only ever
        // attacks dies to bosses a person would comfortably beat. Reach for the
        // biggest heal first; the starter potion can't keep up at tier 4.
        if ((await actingHp()) < 0.55) {
...
async function doTown(rest) {
    ...
    // Stock up on healing. Prefer 大還丹 (+150, stocked from tier 2): the tier-4
    // bosses hit for 52-66, so 金創藥 (+50) loses the race against them and the
    // bot bleeds out no matter how many it carries.
...
async function clearDungeon(id) {
    ...
    if (targets.boss) await walkTo(targets.boss.x, targets.boss.y);
    await drainDialog();
    if (await vis('#battle')) await fight();
    await drainDialog();
