// Play Lumenfall end to end through the real UI and count what a player actually does.
...
async function drainDialog(max = 300) {
    for (let i = 0; i < max; i++) {
        ...
        if (txt) {
            ...
            if (txt.includes('has Faded')) { stats.wipes++; log.push('  *** PARTY WIPED -> respawn at Marrow’s End'); }
            if (!seenDialog.has(txt)) {
                seenDialog.add(txt);
                stats.dialogChars += txt.replace(/[\s\n]|▼ click to continue/g, '').length;
            }
        }
...
const inTargetMode = () =>
    page.locator('#cmds button', { hasText: 'Back' }).first().isVisible().catch(() => false);

// Cast the best healing spell available. Returns true if it took the turn.
async function tryHealSpell() {
    const sp = page.locator('#cmds button', { hasText: 'Spell' }).first();
    ...
    for (const name of ['Full Palette', 'Mend']) {
        ...
        if (await inTargetMode()) {
            ...
            const allies = page.locator('#cmds button').filter({ hasNotText: 'Back' });
    ...
    const back = page.locator('#cmds button', { hasText: 'Back' }).first();
...
async function tryRevive() {
    ...
    if (await sp.isVisible().catch(() => false)) {
        await sp.click();
        await page.waitForTimeout(70);
        const b = page.locator('#cmds button', { hasText: 'Rekindle' }).first();
...
async function fight() {
    ...
    for (let turn = 0; turn < 150; turn++) {
        ...
        if (await inTargetMode()) {
            await page.locator('#cmds button', { hasText: 'Back' }).first().click().catch(() => {});
        ...
        if (hurt < 0.55) {
            const items = page.locator('#cmds button', { hasText: 'Item' }).first();
            if (await items.isVisible().catch(() => false)) {
                await items.click().catch(() => {});
                await page.waitForTimeout(60);
                let potion = page.locator('#cmds button', { hasText: 'Rich Salve' }).first();
                if (!(await potion.isVisible().catch(() => false)))
                    potion = page.locator('#cmds button', { hasText: 'Salve' }).first();
                if (await potion.isVisible().catch(() => false)) {
                    ...
                    const ally = page.locator('#cmds button').filter({ hasNotText: 'Back' }).first();
        ...
        const atk = page.locator('#cmds button', { hasText: 'Strike' }).first();
...
async function doTown(rest) {
    ...
    for (let i = 0; i < 4; i++) {
        ...
        if (!(await clickTown('Talk to the locals'))) break;
        await drainDialog();
    ...
    if (rest) {
        if (await clickTown('Inn')) { stats.innRests++; await drainDialog(); }
    }
    if (await clickTown('Shop')) {
        ...
        for (const name of ['Rich Salve', 'Salve']) {
            for (let i = 0; i < 10; i++) {
...
// ================= run =================
try {
    await page.goto('http://localhost:8765/lumenfall/');
    await page.evaluate(() => localStorage.removeItem('lumenfallSave'));
    await page.reload();
    ...
    await walkTo(t1.x, t1.y); await doTown(true); await mark('town1 Marrow');

    await goDungeon('A', 'earth'); await mark('Ashwood');

    const t2 = await findWorld('2');
    await walkTo(t2.x, t2.y); await doTown(true); await mark('town2 Hollowmere');

    await goDungeon('B', 'fire'); await mark('Emberhollow');

    const t3 = await findWorld('3');
    await walkTo(t3.x, t3.y); await doTown(true); await mark('town3 Kiln Row');

    await goDungeon('C', 'water'); await mark('Sunken Gallery');

    // the tomb and the castle are full dungeons with bosses, not just map events
    await goDungeon('J', 'tomb'); await mark('Palette Vault (optional)');

    await goDungeon('K', 'castle'); await drainDialog(400); await mark('the Wellspring');

    const ending = await vis('#ending') || (await objective()).includes('color');
